...
class TestModels(unittest.TestCase):
    ...
    def setUp(self):
        self.app = app
        self.app.config['SQLALCHEMY_DATABASE_URI'] = 'postgresql://postgres:utec1719@localhost/software'
        self.app.config['SQLALCHEMY_TRACK_MODIFICATIONS'] = False
        self.app.config['TESTING'] = True

        with self.app.app_context():
            db.create_all()
            self.cuenta1 = Cuentausuario(numero='21345', nombre='Arnoldo')
            self.cuenta2 = Cuentausuario(numero='123', nombre='Luisa')
            self.cuenta3 = Cuentausuario(numero='456', nombre='Andrea')
